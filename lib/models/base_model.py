...
class BaseModel(AbstractModel):
    ...
    def _get_hoi_branch(self):
        return SimpleHOIModule(self.mask_rcnn_vis_feat_dim, 2 * self.obj_rnn_emb_dim, self.spatial_emb_dim)


class SimpleHOIModule(nn.Module):
    def __init__(self, vis_feats_dim, obj_ctx_dim, spatial_ctx_dim, **kwargs):
        ...
        self.rel_output_emb_fc = nn.Sequential(*(
                ...
                +
                [nn.Linear(self.rel_vis_hidden_dim + obj_ctx_dim + 2 * spatial_ctx_dim, self.rel_hidden_dim),
    ...
    def forward(self, x, **kwargs):
        ...
        with torch.set_grad_enabled(self.training):
            rel_feats = self._forward(union_boxes_feats, box_feats, spatial_ctx, obj_ctx, im_ids, hoi_im_ids, sub_inds, obj_inds)
            return rel_feats
    ...
    def _forward(self, union_boxes_feats, box_feats, spatial_ctx, obj_ctx, unique_im_ids, hoi_im_ids, sub_inds, obj_inds):
        ...
        obj_ctx_rep = torch.cat([obj_ctx[i, :].expand((hoi_im_ids == im_id).sum(), -1) for i, im_id in enumerate(unique_im_ids)], dim=0)
        spatial_rel_ctx_rep = torch.cat([spatial_ctx[i, :].expand((hoi_im_ids == im_id).sum(), -1) for i, im_id in enumerate(unique_im_ids)], dim=0)
