...
class BaseModel(AbstractModel):
    ...
    def _get_hoi_branch(self):
        return SimpleHOIModule(self.mask_rcnn_vis_feat_dim, self.obj_branch.output_ctx_dim, self.spatial_context_branch.output_dim)
...
class SimpleHOIModule(nn.Module):
    def __init__(self, vis_feats_dim, obj_ctx_dim, spatial_ctx_dim, **kwargs):
        ...
        self.rel_output_emb_fc = nn.Sequential(*(
                ...
                +
                [nn.Linear(self.rel_vis_hidden_dim + obj_ctx_dim + spatial_ctx_dim, self.rel_hidden_dim),
