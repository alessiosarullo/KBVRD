...
def arange2d(base_tensor, dim=0):
    # `dim` is the dimension along which the range is taken. It is repeated across the other dimension.
    assert dim == 0 or dim == 1
    new_size = base_tensor.size(dim)
    new_vec = base_tensor.new(new_size).long()
    torch.arange(0, new_size, out=new_vec)
    return new_vec.view(-1 if dim == 0 else 1, -1 if dim == 1 else 1).expand(base_tensor.size(0), base_tensor.size(1))


def to_onehot(vec, num_classes, fill=1000, polar_repr=True):
    """
    Creates a [size(vec), num_classes] torch FloatTensor where one_hot[i, vec[i]] = fill and the rest is either -fill or 0.
    
    :param vec: 1d torch tensor
    :param num_classes: int
    :param fill: value that we want + (and -, if using polar representation) things to be.
    :param polar_repr: whether to use polar representation (+fill/-fill instead of +fill/0)
    :return: 
    """
    onehot_result = vec.new(vec.size(0), num_classes).float().fill_(-fill if polar_repr else 0)
