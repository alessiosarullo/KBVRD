...
class ExtModel(HoiModel):
    ...
    def _forward(self, boxes_ext, box_feats, masks, union_boxes_feats, hoi_infos, box_labels=None, hoi_labels=None):
        obj_logits, hoi_logits_raw = super()._forward(boxes_ext, box_feats, masks, union_boxes_feats, hoi_infos, box_labels, hoi_labels)

        hoi_infos = torch.tensor(hoi_infos, device=masks.device)

        hoi_logits = self.ext_branch(hoi_logits_raw, obj_logits, boxes_ext, hoi_infos)
