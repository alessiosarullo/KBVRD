...
class Pic:
    def __init__(self):
        self.data_dir = os.path.join('data', 'PiC')
    ...
    def _load(self):
        with open(os.path.join(self.data_dir, 'relations_train.json'), 'r') as f:
            train = json.load(f)  # List of {'image_id', 'name', 'relations'}

        with open(os.path.join(self.data_dir, 'relations_test.json'), 'r') as f:
            test = json.load(f)  # List of {'image_id', 'name', 'relations'}

        with open(os.path.join(self.data_dir, 'label_categories.txt'), 'r') as f:
            self.objects = [l.strip() for l in f.readlines()]

        with open(os.path.join(self.data_dir, 'relation_categories.txt'), 'r') as f:
            self.predicates = [l.strip() for l in f.readlines()]
