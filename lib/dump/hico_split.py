            ...
            if self.split != Splits.TEST:
                ...
                if cfg.mlneg:
                    ...
                    for i in range(labels.shape[0]):
                        ...
                        label_mask[i, np.random.choice(negs, size=min(num_negs, cfg.hoi_bg_ratio * num_pos), replace=False)] = True
                    label_mask = torch.tensor(label_mask.astype(np.uint8), device=device)
